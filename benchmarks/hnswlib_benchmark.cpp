...
int main(){
    ...
    // We align the parameters with your custom Engine
    int M = 32;
    int ef_construction = 400;
    int ef_search = 200;
    
    cout<<"INITIALIZING HNSWLIB INDEX..."<<endl;
    hnswlib::L2Space space(dim);
    hnswlib::HierarchicalNSW<float> db(&space, base_vecs.size(), M, ef_construction);
    
    cout<<"STARTING INSERTION..."<<endl;
    auto start_insert=chrono::high_resolution_clock::now();
    for(size_t i=0;i<base_vecs.size();i++){
        db.addPoint(base_vecs[i].data(), i);
        if((i+1)%100000==0){
    ...
    for(size_t i=0;i<query_vecs.size();i++){
        ...
        auto res=db.searchKnn(query_vecs[i].data(), k_test);
        ...
        vector<int> retrieved_ids;
        while (!res.empty()) {
            retrieved_ids.push_back(res.top().second);
            res.pop();
        }
        reverse(retrieved_ids.begin(), retrieved_ids.end());

        for(size_t j=0;j<retrieved_ids.size();j++){
            int id=retrieved_ids[j];
    ...
    cout<<"\n==========================================="<<endl;
    cout<<"       HNSWLIB SIFT1M BENCHMARK RESULTS      "<<endl;
    cout<<"==========================================="<<endl;
